...
public class UtilGrafos {
    ...
    public static Comparable[][] convertirWarshall(boolean[][] warshall){
        Comparable[][] warshall2 = new Comparable[warshall.length][warshall.length];
    ...
    public static void imprimirBpf(Collection<TVertice> recorrido) {
        int indice = 0;
        for (TVertice etVert : recorrido) {
            if (indice != recorrido.size()-1) {
                System.out.print(etVert.getEtiqueta() + " --> ");
            } else {
                System.out.println(etVert.getEtiqueta());
            }
            indice++;
        }
    }
