...
public class UtilGrafos {
    ...
    /**
     * Carga un grafo dirigido desde archivos de texto.
     * Los archivos deben contener los vértices y las aristas en un formato específico.
     * El archivo de vértices debe contener una lista de etiquetas de vértices,
     * y el archivo de aristas debe contener líneas con el formato: origen, destino, costo.
     * @param <T> el tipo de los datos almacenados en los vértices del grafo
     * @param <G> el tipo del grafo que se va a cargar
     * @param <V> el tipo de los vértices del grafo
     * @param nomArchVert nombre del archivo que contiene los vértices del grafo
     * @param nomArchAdy   nombre del archivo que contiene las aristas del grafo
     * @param ignoreHeader indica si se debe ignorar la primera línea del archivo (cabecera)
     * @param claseDelGrafo la clase del grafo que se va a cargar
     * @param claseDelVertice la clase de los vértices que se van a cargar
     * @return
     */
            ...
            if ((verticeString != null) && (verticeString.trim() != "")) {
                try {
                    if (claseDelVertice != null) {
                        ...
                        // Se espera que cada línea del archivo de vértices contenga una etiqueta y opcionalmente otros parámetros separados por comas.
                        // Por ejemplo: "A,1,2,3" para un vértice con etiqueta "A" y parámetros adicionales "1", "2", "3".
                        // Se asume que el constructor variádico acepta un array de objetos.
                        // Si no existe, se usa el constructor que toma una Comparable.
                        String[] parametrosDelVertice = verticeString.split(",");
                        try {
                            // Try to get the variadic constructor
                            constructor = claseDelVertice.getConstructor(Object[].class);
                            vertice = constructor.newInstance(new Object[] { parametrosDelVertice });
                        } catch (NoSuchMethodException e) {
                            // If the variadic constructor doesn't exist, fall back to the current one
                            verticeString = parametrosDelVertice[0];
                        ...
                        verticesList.add(new TVertice<T>(verticeString, null));
                    }
                } catch (Exception ex) {
